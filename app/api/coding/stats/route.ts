...
/**
 * GET /api/coding/stats - Get coding statistics for an exam
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const stats = await CodingService.getCodingStats(user.id, examId);

    return NextResponse.json({
      data: stats,
    });
  } catch (error) {
    console.error("GET /api/coding/stats error:", error);
